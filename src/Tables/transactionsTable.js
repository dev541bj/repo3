...
class TransactionsTable extends React.Component {
  ...
  async updateTableContent() {
    try {
      ...
      const transData = transactionsResp.data.data;
  ...
  handleSelectAllClick = event => {
    if (event.target.checked) {
      this.setState(
        state => ({ selected: state.transData.map(n => n.id) }),
  ...
  render() {
    ...
    const emptyRows =
      rowsPerPage -
      Math.min(rowsPerPage, transData.length - page * rowsPerPage);
          ...
          <div className={classes.tableWrapper}>
            <Table /* className={classes.table} */ aria-labelledby="tableTitle">
              <TransactionsTableHead
                ...
                rowCount={transData.length || 0}
              />
              <TableBody>
                {stableSort(transData, getSorting(order, orderBy))
